...
% Inputs are 
% Output is 
function traj = TrajectoryGeneratorNewTask(Tse_initial, Tsc_initial, Tsc_final, Tce_grasp, Tce_standoff, Tse_safe1, Tse_safe2, k)

    t = 2;
    N = t*k/(0.01);

    %adding safe move
    traj_safe1 = ScrewTrajectory(Tse_initial, Tse_safe1, t, N, 5);
    traj_safe1_matrix = zeros(N,13);
    gripper_state = 0;
    for i = 1:N
        traj_safe1_matrix(i,:) = [traj_safe1{i}(1,1),traj_safe1{i}(1,2), traj_safe1{i}(1,3), traj_safe1{i}(2,1), traj_safe1{i}(2,2), traj_safe1{i}(2,3), traj_safe1{i}(3,1), traj_safe1{i}(3,2), traj_safe1{i}(3,3), traj_safe1{i}(1,4), traj_safe1{i}(2,4), traj_safe1{i}(3,4), gripper_state];
    end
    
    % move 1
    Tse_standoff_initial = Tsc_initial*Tce_standoff;
    traj1 = ScrewTrajectory(Tse_safe1, Tse_standoff_initial, t, N, 5);
    ...
    %adding safe move
    traj_safe2 = ScrewTrajectory(Tse_standoff_initial, Tse_safe2, t, N, 5);
    traj_safe2_matrix = zeros(N,13);
    for i = 1:N
        traj_safe2_matrix(i,:) = [traj_safe2{i}(1,1),traj_safe2{i}(1,2), traj_safe2{i}(1,3), traj_safe2{i}(2,1), traj_safe2{i}(2,2), traj_safe2{i}(2,3), traj_safe2{i}(3,1), traj_safe2{i}(3,2), traj_safe2{i}(3,3), traj_safe2{i}(1,4), traj_safe2{i}(2,4), traj_safe2{i}(3,4), gripper_state];
    end

    % move 5
    Tse_standoff_final = Tsc_final*Tce_standoff;
    traj5 = ScrewTrajectory(Tse_safe2, Tse_standoff_final, t, N, 5);
    ...
    traj = [traj_safe1_matrix; traj1_matrix; traj2_matrix; traj3_matrix; traj4_matrix; traj_safe2_matrix; traj5_matrix; traj6_matrix; traj7_matrix; traj8_matrix];
end
